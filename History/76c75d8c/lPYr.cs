...
namespace WinFormsApp1.Clases
{
    internal class Clase_Profesores
    {
        ...
        public void ConsultaProfesores(TextBox ID,TextBox Nom, TextBox Ape, TextBox Date)
        {
            try
            {
                cadena objConex = new cadena();
                string query = "SELECT ID_PROFESOR, NOMBRE, APELLIDOS, F_NACIMIENTO FROM ALUMNOS WHERE " +
                "ID_PROFESOR = " + ID.Text + ";";
