...
if(isset($_POST['Registrar'])){
    if(strlen($_POST['nombre']) >= 1 && strlen($_POST['apellido']) >= 1 && strlen($_POST['email']) >= 1 && strlen($_POST['contraseña']) >= 1 && strlen($_POST['verificar-contraseña']) >= 1)
    {
        ...
        if ($contraseña == $validar_contraseña)
        {
            ...
            //auto incremento
            $id_usuario_consulta="SELECT MAX(id_usuario) + 1 AS next_id FROM usuarios;";
            $row = mysqli_fetch_assoc($id_usuario_consulta);
            $id_usuario=mysqli_query($conexion, $id_usuario_consulta);
            $consulta="INSERT INTO `usuarios`(`id_usuario`, `nombre_usuario`, `apellido_usuario`, `contraseña`, `correo_electronico`) VALUES ('$id_usuario','$nombre','$apellido','$email','$contraseña_encriptada');";
