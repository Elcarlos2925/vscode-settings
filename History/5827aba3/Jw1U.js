const productos=[
    {
        "Id": "1",
        "Nombre": "Café de Veracruz",
        "Precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    },
    {
        "id": "1",
        "titulo": "Café de Veracruz 300g",
        "imagen": "./img/C1.png",
        "precio": 250.00,
        "Descripción": `<br>Descripción
        <br>Descubre la rica tradición cafetera de Veracruz en cada taza con nuestro Café de Veracruz, una joya de la región tostada a la perfección. Este café de especialidad veracruzano está elaborado con gran pasión y cuidado. Nuestra selección de granos 100% arábigo brinda una experiencia única, con un tostado medio que resalta sus sabores naturales.
        
        <br>Este café es más que una bebida, es una conexión con la tierra, la cultura y la pasión que Veracruz pone en cada grano. Aprovecha la oportunidad de saborear una taza única de México en cualquier momento del día. ¡Adquiere tu bolsa de Café de Veracruz hoy y lleva a tu hogar el auténtico sabor de esta tierra mágica!
        
        <br>Contenido neto: 300 g (gramos)
        <br>Intensidad: 6
        <br>Tostado: Medio
        <br>Aroma: Intenso
        <br>Intensidad: Ligero
        <br>Notas: Afrutadas
        <br>Molienda: Media
        `, 
        "Ingredientes": `<br>Ingredientes
        <br>Café de especialidad veracruzano tostado medio y molido 100% arábigo. 
        `,
        "Instrucciones de uso": `<br>Instrucciones de uso
        <br>Almacenamiento. Guarda tu bolsa de café en un lugar fresco, oscuro y seco. Evita la exposición a la luz directa y al aire, ya que estos factores pueden afectar la frescura y calidad del café.
        <br>
        <br>Proporciones. Asegúrate de utilizar la cantidad adecuada de café por cada taza. Las recomendaciones varían según el método de preparación, pero generalmente se usa una cucharada de café por cada 180 ml de agua como punto de partida. Ajusta la cantidad según tu preferencia personal.
        <br>
        <br>Preparación
        <br>Paso 1. Añade una cucharadita de café en tu taza. Si prefieres endulzarlo, puedes agregar azúcar al gusto.
        <br>Paso 2. Vierte cuidadosamente agua caliente en la taza, dejando un pequeño espacio en la parte superior. Remueve adecuadamente para asegurarte de que tanto el café como el azúcar se disuelva completamente (esto debería tomar aproximadamente 10-15 segundos).
        <br>Paso 3. Si te gusta tu café con leche, este es el momento ideal para agregarla.
        <br>Paso 4. Prueba tu café. ¿No tiene la fuerza deseada? Añade más gránulos de café la próxima vez. ¿Demasiado fuerte? Reduce la cantidad de café y añade un poco más de leche. Ajusta según sean tus preferencias.
        `
    },

    {
        "id": "2",
        "titulo": "Café de Puebla 300g",
        "imagen": "./img/C3.png",
        "precio": 250.00,
        "Descripción": `
        <br>Descripción
        <br>Sumérgete en el mundo del café poblano con nuestro Café de Puebla, una verdadera joya tostada con mucho cariño en esta hermosa región.  Este café de especialidad poblano es el fruto de la pasión y dedicación que los amantes del café han vertido en su creación. Hemos elegido granos 100% arábigo para ofrecer una experiencia única, con un tostado medio que resalta sus sabores naturales.
        <br>
        <br>Este café te lleva directo a la tierra y la cultura de Puebla. Aprovecha la oportunidad de disfrutar de una taza única de México en cualquier momento del día. ¡Consigue tu bolsa de Café de Puebla hoy y lleva a tu casa el auténtico sabor de esta tierra rica en tradición.
        <br>
        <br>Contenido neto: 300 g (gramos)
        <br>Intensidad: 6
        <br>Tostado: Medio
        <br>Aroma: Intenso
        <br>Intensidad: Ligera
        <br>Notas: Afrutadas
        <br>Molienda: Media
        `, 
        "Ingredientes": `
        <br>Ingredientes
        <br>Café de especialidad poblano tostado medio y molido 100% arábigo.
        `,
        "Instrucciones de uso": `
        <br>Instrucciones de uso
        <br>Almacenamiento. Guarda tu bolsa de café en un lugar fresco, oscuro y seco. Evita la exposición a la luz directa y al aire, ya que estos factores pueden afectar la frescura y calidad del café.
        <br>
        <br>Proporciones. Asegúrate de utilizar la cantidad adecuada de café por cada taza. Las recomendaciones varían según el método de preparación, pero generalmente se usa una cucharada de café por cada 180 ml de agua como punto de partida. Ajusta la cantidad según tu preferencia personal. 
        <br>
        <br>Preparación
        <br>Paso 1. Añade una cucharadita de café en tu taza. Si prefieres endulzarlo, puedes agregar azúcar al gusto.
        <br>Paso 2. Vierte cuidadosamente agua caliente en la taza, dejando un pequeño espacio en la parte superior. Remueve adecuadamente para asegurarte de que tanto el café como el azúcar se disuelva completamente (esto debería tomar aproximadamente 10-15 segundos).
        <br>Paso 3. Si te gusta tu café con leche, este es el momento ideal para agregarla.
        <br>Paso 4. Prueba tu café. ¿No tiene la fuerza deseada? Añade más gránulos de café la próxima vez. ¿Demasiado fuerte? Reduce la cantidad de café y añade un poco más de leche. Ajusta según sean tus preferencias.
        `
    },

    {
        "id": "3",
        "titulo": "Café de Chiapas 300g",
        "imagen": "./img/C5.png",
        "precio": 250.00,
        "Descripción": `
        <br>Descripción
        <br>Adéntrate en el mundo del café chiapaneco con nuestro Café de Chiapas, una auténtica joya tostada con cariño en esta hermosa región. Este café especial de Chiapas es el resultado de la pasión y el esfuerzo dedicado por los amantes del café en su creación. Hemos seleccionado granos 100% arábigos para brindar una experiencia única, con un tostado medio que realza sus sabores naturales.
        <br>
        <br>Este café te transporta directamente a la tierra y la cultura de Chiapas. Aprovecha la oportunidad de saborear una taza única de México en cualquier momento del día. ¡Obtén tu bolsa de Café de Chiapas hoy y lleva a tu hogar el auténtico sabor de esta tierra rica en tradición.
        <br>
        <br>Contenido neto: 300 g (gramos)
        <br>Intensidad: 6
        <br>Tostado: Medio
        <br>Aroma: Intenso
        <br>Intensidad: Ligera
        <br>Notas: Afrutadas
        <br>Molienda: Media
        `, 
        "Ingredientes": `
        <br>Ingredientes
        <br>Café de especialidad chiapaneco tostado medio y molido 100% arábigo. 
        `,
        "Instrucciones de uso": `
        <br>Instrucciones de uso
        <br>Almacenamiento. Guarda tu bolsa de café en un lugar fresco, oscuro y seco. Evita la exposición a la luz directa y al aire, ya que estos factores pueden afectar la frescura y calidad del café.
        <br>
        <br>Proporciones. Asegúrate de utilizar la cantidad adecuada de café por cada taza. Las recomendaciones varían según el método de preparación, pero generalmente se usa una cucharada de café por cada 180 ml de agua como punto de partida. Ajusta la cantidad según tu preferencia personal.
        <br>
        <br>Preparación
        <br>Paso 1. Añade una cucharadita de café en tu taza. Si prefieres endulzarlo, puedes agregar azúcar al gusto.
        <br>Paso 2. Vierte cuidadosamente agua caliente en la taza, dejando un pequeño espacio en la parte superior. Remueve adecuadamente para asegurarte de que tanto el café como el azúcar se disuelva completamente (esto debería tomar aproximadamente 10-15 segundos).
        <br>Paso 3. Si te gusta tu café con leche, este es el momento ideal para agregarla.
        <br>Paso 4. Prueba tu café. ¿No tiene la fuerza deseada? Añade más gránulos de café la próxima vez. ¿Demasiado fuerte? Reduce la cantidad de café y añade un poco más de leche. Ajusta según sean tus preferencias.
        `
    },

    {
        "id": "4",
        "titulo": "Café de Oaxaca 300g",
        "imagen": "./img/C7.png",
        "precio": 250.00,
        "Descripción": `
        <br>Descripción
        <br>Adéntrate en el apasionante mundo del café oaxaqueño con nuestro Café de Oaxaca, una auténtica joya tostada con cariño en esta hermosa región. Este café especial de Oaxaca es el resultado del esfuerzo y dedicación de los amantes del café en su creación. Hemos seleccionado granos 100% arábigo para brindar una experiencia única, con un tostado medio que realza sus sabores naturales.
        <br>
        <br>Este café te lleva directamente a la tierra y la cultura de Oaxaca. Aprovecha la oportunidad de saborear una taza única de México en cualquier momento del día. ¡Obtén tu bolsa de Café de Oaxaca hoy y lleva a tu hogar el auténtico sabor de esta tierra rica en tradición.
        <br>
        <br>Contenido neto: 300 g (gramos)
        <br>Intensidad: 6
        <br>Tostado: Medio
        <br>Aroma: Intenso
        <br>Intensidad: Ligera
        <br>Notas: Afrutadas
        <br>Molienda: Media
        `, 
        "Ingredientes": `
        <br>Ingredientes
        <br>Café de especialidad oaxaqueño tostado medio y molido 100% arábigo.
        `,
        "Instrucciones de uso": `
        <br>Instrucciones de uso
        <br>Almacenamiento. Guarda tu bolsa de café en un lugar fresco, oscuro y seco. Evita la exposición a la luz directa y al aire, ya que estos factores pueden afectar la frescura y calidad del café.
        <br>
        <br>Proporciones. Asegúrate de utilizar la cantidad adecuada de café por cada taza. Las recomendaciones varían según el método de preparación, pero generalmente se usa una cucharada de café por cada 180 ml de agua como punto de partida. Ajusta la cantidad según tu preferencia personal.
        <br>
        <br>Preparación
        <br>Paso 1. Añade una cucharadita de café en tu taza. Si prefieres endulzarlo, puedes agregar azúcar al gusto.
        <br>Paso 2. Vierte cuidadosamente agua caliente en la taza, dejando un pequeño espacio en la parte superior. Remueve adecuadamente para asegurarte de que tanto el café como el azúcar se disuelva completamente (esto debería tomar aproximadamente 10-15 segundos).
        <br>Paso 3. Si te gusta tu café con leche, este es el momento ideal para agregarla.
        <br>Paso 4. Prueba tu café. ¿No tiene la fuerza deseada? Añade más gránulos de café la próxima vez. ¿Demasiado fuerte? Reduce la cantidad de café y añade un poco más de leche. Ajusta según sean tus preferencias.
        `
    },

    {
        "id": "5",
        "titulo": "Café especial 300g",
        "imagen": "./img/C11.png",
        "precio": 250.00,
        "Descripción": `
        <br>Descripción
        <br>Sumérgete en el maravilloso mundo del café con nuestro Café Especial The Drop Coffee. Este café es el corazón de nuestra casa, una creación única que te ofrece una experiencia de café excepcional. Hemos seleccionado los granos más finos y excepcionales para brindarte un sabor verdaderamente distintivo.
        <br>
        <br>Cada taza de Café Especial The Drop Coffee es una aventura de sabores, con detalles únicos que te transportan a lugares lejanos. Nuestro café está tostado a la perfección para resaltar su carácter especial, ofreciendo un equilibrio entre intensidad y suavidad que te encantará en cada sorbo.
        <br>
        <br>Este café es más que una bebida, es una expresión de nuestra pasión por el café de calidad. Disfruta de una taza de The Drop Coffee en cualquier momento del día y descubre por qué es un verdadero café especial de la casa.
        <br>
        <br>Contenido neto: 300 g (gramos)
        <br>Intensidad: 6
        <br>Tostado: Medio
        <br>Aroma: Intenso
        <br>Intensidad: Ligera
        <br>Notas: Afrutadas
        <br>Molienda: Media
        `, 
        "Ingredientes": `
        <br>Ingredientes
        <br>Café de especialidad The Drop Coffee tostado medio y molido 100% arábigo. 
        `,
        "Instrucciones de uso": `
        Instrucciones de uso

        Almacenamiento. Guarda tu bolsa de café en un lugar fresco, oscuro y seco. Evita la exposición a la luz directa y al aire, ya que estos factores pueden afectar la frescura y calidad del café.
        
        Proporciones. Asegúrate de utilizar la cantidad adecuada de café por cada taza. Las recomendaciones varían según el método de preparación, pero generalmente se usa una cucharada de café por cada 180 ml de agua como punto de partida. Ajusta la cantidad según tu preferencia personal.
        
        
        Preparación
        Paso 1. Añade una cucharadita de café en tu taza. Si prefieres endulzarlo, puedes agregar azúcar al gusto.
        Paso 2. Vierte cuidadosamente agua caliente en la taza, dejando un pequeño espacio en la parte superior. Remueve adecuadamente para asegurarte de que tanto el café como el azúcar se disuelva completamente (esto debería tomar aproximadamente 10-15 segundos).
        Paso 3. Si te gusta tu café con leche, este es el momento ideal para agregarla.
        Paso 4. Prueba tu café. ¿No tiene la fuerza deseada? Añade más gránulos de café la próxima vez. ¿Demasiado fuerte? Reduce la cantidad de café y añade un poco más de leche. Ajusta según sean tus preferencias.
        `
    },

    {
        "id": "6",
        "titulo": "Café mezclas <br> mexicanas 300g",
        "imagen": "./img/C9.png",
        "precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    },

    {
        "id": "7",
        "titulo": "Café de Veracruz Descafeinado 300g",
        "imagen": "./img/C2.png",
        "precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    },

    {
        "id": "8",
        "titulo": "Café de Puebla Descafeinado 300g",
        "imagen": "./img/C4.png",
        "precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    },

    {
        "id": "9",
        "titulo": "Café de Chiapas Descafeinado 300g",
        "imagen": "./img/C6.png",
        "precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    },

    {
        "id": "10",
        "titulo": "Café de Oaxaca Descafeinado 300g",
        "imagen": "./img/C8.png",
        "precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    },

    {
        "id": "11",
        "titulo": "Café mezclas mexicanas Descafeinado 300g",
        "imagen": "./img/C10.png",
        "precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    },

    {
        "id": "12",
        "titulo": "Café especial <br> Descafeinado 300g",
        "imagen": "./img/C12.png",
        "precio": 250.00,
        "Descripción": ``, 
        "Ingredientes": ``,
        "Instrucciones de uso": ``
    }
]