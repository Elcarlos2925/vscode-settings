...
function numeroTarjeta(input) {
    // Eliminar espacios en blanco y no números
  var formattedInput = input.replace(/\D/g, '');
  
  // Dividir el número en grupos de 4
  var parts = formattedInput.match(/[\s\S]{1,4}/g) || [];
  
  // Unir los grupos con espacios en blanco
  var formattedNumber = parts.join(' ');
  
  // Asignar el número formateado al campo de entrada
  document.getElementById('numeroTarjeta').value = formattedNumber;
}
