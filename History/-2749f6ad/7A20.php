...
if(isset($_POST['guardar-direccion']))
{
    ...
    >= 1)
    {
        ...
        if ($row !== null && array_key_exists('next_id', $row)) {
            $id_direccion = $row['next_id'];
        } else {
            // Asigna un valor predeterminado si la consulta es nula o no contiene 'next_id'
            $id_direccion = 1; // Cambia esto al valor predeterminado que desees
        }
        $consulta="INSERT INTO `usuarios`(`id_direccion`, `nombre_usuario`, `apellido_usuario`, `contraseña`, `correo_electronico`) VALUES ('$id_direccion','$nombre','$apellido','$contraseña_encriptada','$email');";
        $consulta="INSERT INTO `direccion`(`codigo_postal`, `ciudad`, `estado`, `calle`, `entre_calle`, `especificaciones`, `num_int`, `num_ext`, `id_direccion`, `nombre_receptor`, `tel`, `id_usuario`) VALUES 
                                            ('$CP','$municipio]','$estado','$calle','$entre_calles','$especificaciones','$num_int','$num_ext','$id_direccion','$nombre','$telefono','id_usuario')";
        $resultado=mysqli_query($conexion, $consulta);
        $mensaje="<div class='contenedor-mensaje-succes'>
                <strong>Dirección guardada exitosamente.</strong>
                </div>";
    }
    else{
